...
import { CalendarClock, TrendingUp, CheckCircle2, Clock } from 'lucide-react';
  ...
  return (
    <div className={`rounded-xl border p-4 space-y-3 ${st.bg}`}>
      <div className="flex items-center justify-between">
        ...
        <span className={`badge text-xs font-medium ${st.color} bg-white/5`}>{st.label}</span>
      </div>

      <div className="grid grid-cols-3 gap-3">
        {proj.totalExpected !== null && (
          <div className="text-center">
            <p className="text-xs text-white/40 mb-0.5">Total Expected</p>
            <p className={`text-base font-bold ${accentColor}`}>{formatCurrency(proj.totalExpected)}</p>
            <p className="text-xs text-white/30">{proj.totalPeriods} {unit}</p>
          </div>
        )}
        <div className="text-center">
          <p className="text-xs text-white/40 mb-0.5">Elapsed</p>
          <p className="text-base font-bold text-white/70">{formatCurrency(proj.elapsedAmount)}</p>
          <p className="text-xs text-white/30">{proj.elapsedPeriods} {unit}</p>
        </div>
        <div className="text-center">
          <p className="text-xs text-white/40 mb-0.5">Remaining</p>
          {proj.remainingAmount !== null ? (
            <>
              <p className={`text-base font-bold ${proj.remainingAmount > 0 ? 'text-yellow-400' : 'text-white/30'}`}>
                {formatCurrency(proj.remainingAmount)}
              </p>
              <p className="text-xs text-white/30">{proj.remainingPeriods} {unit}</p>
            </>
          ) : (
            <p className="text-sm text-white/30">Ongoing</p>
          )}
        </div>
      </div>
