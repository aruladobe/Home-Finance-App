...
function ResultCard({ label, value, color = 'primary' }) {
  const colors = {
    primary: 'from-primary-500/15 to-purple-600/10 border-primary-500/20 text-primary-400',
    green:   'from-emerald-500/15 to-emerald-600/10 border-emerald-500/20 text-emerald-400',
    red:     'from-red-500/15 to-red-600/10 border-red-500/20 text-red-400',
    amber:   'from-amber-500/15 to-amber-600/10 border-amber-500/20 text-amber-400',
    purple:  'from-purple-500/15 to-purple-600/10 border-purple-500/20 text-purple-400',
    cyan:    'from-cyan-500/15 to-cyan-600/10 border-cyan-500/20 text-cyan-400',
  };
  const [grad, valueColor] = colors[color].split(' text-');
  return (
    <div className={`glass-card p-5 bg-gradient-to-br ${grad} text-${valueColor}`}>
        ...
        <div className="space-y-4 animate-fade-in">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="glass-card p-5 bg-gradient-to-br from-primary-500/15 to-purple-600/10 border-primary-500/20">
              ...
              <p className="text-2xl font-bold text-primary-400">{formatCurrency(result.emi)}</p>
            </div>
            <div className="glass-card p-5 bg-gradient-to-br from-emerald-500/15 to-emerald-600/10 border-emerald-500/20">
              <p className="text-xs text-white/50 mb-1">Total Amount Payable</p>
          ...
          <div className="glass-card overflow-hidden">
            <div className="px-5 py-3 border-b border-white/10 flex items-center justify-between">
              <h4 className="text-sm font-semibold text-white/70">Amortization Schedule</h4>
              <span className="text-xs text-white/30">Showing first 12 months</span>
            </div>
                ...
                <tbody>
                  {result.schedule.slice(0, 12).map(row => (
                    <tr key={row.month} className="table-row">
                      <td className="px-5 py-2.5 text-white/60">{row.month}</td>
                      <td className="px-5 py-2.5 text-right text-white">{formatCurrency(row.emi)}</td>
                      ...
                      <td className="px-5 py-2.5 text-right text-white/60">{formatCurrency(row.balance)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-4 animate-fade-in">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="glass-card p-5 bg-gradient-to-br from-amber-500/15 to-amber-600/10 border-amber-500/20">
              <p className="text-xs text-white/50 mb-1">Simple Interest</p>
            ...
            <div className="glass-card p-5 bg-gradient-to-br from-blue-500/15 to-blue-600/10 border-blue-500/20">
              <p className="text-xs text-white/50 mb-1">Monthly Rate</p>
              <p className="text-2xl font-bold text-blue-400">{result.ratePerMonth.toFixed(4)}%</p>
            </div>
          </div>
          <div className="glass-card p-5">
            <p className="text-xs text-white/40 font-mono">SI = (P × R × T) / 100 = ({formatCurrency(Number(form.principal))} × {form.rate}% × {(Number(form.tenure)/12).toFixed(2)} yrs) / 100 = {formatCurrency(result.si)}</p>
          </div>
        </div>
      )}
    </div>
        ...
        <div className="space-y-4 animate-fade-in">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="glass-card p-5 bg-gradient-to-br from-purple-500/15 to-purple-600/10 border-purple-500/20">
              <p className="text-xs text-white/50 mb-1">Compound Interest</p>
    ...
    <div className="space-y-6">
        ...
        {MODES.map(m => (
          <button key={m.id} onClick={() => { setMode(m.id); setResult(null); setA(''); setB(''); }}
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all border ${
              mode === m.id
                ? 'bg-primary-600/30 text-white border-primary-500/30'
      ...
      {result && (
        <div className="glass-card p-6 bg-gradient-to-br from-cyan-500/15 to-cyan-600/10 border-cyan-500/20 animate-fade-in">
          <p className="text-xs text-white/50 mb-2">{result.label}</p>
...
export default function CalculatorPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h2 className="text-xl font-bold text-white">Financial Calculator</h2>
        <p className="text-sm text-white/40 mt-1">EMI, interest & percentage calculations</p>
      </div>
        ...
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${
              activeTab === id
                ? 'bg-gradient-to-r from-primary-600/40 to-purple-600/30 text-white border border-primary-500/30'
